...
plot = ggplot(data=data, aes(label=State))+
  geom_point(aes(y=O3, x=AQI))+
  ggtitle('Average O3 by US State in 2015')+ 
  labs(y='O3 (Parts per billion)', x='O3 AQI')+
  theme_economist()
ggplotly(plot)
```
...
pal <- colorNumeric(
  palette = "Blues",
  domain = recent_data$o3_aqi)

recent_data %>%
  leaflet() %>%
  addTiles() %>%
  addCircleMarkers(label = ~State,
                   popup = ~paste(round(o3, 4), 'parts per billion<br>AQI: ', 
                                  round(o3_aqi)),
                   color = ~pal(o3_aqi),
                   opacity = 0.8,
                   radius = ~o3*500,
...
![](/post/2020-11-17-ozone_files/O3_AQI_Worst_States.png)
While the majority of readings stay under 50, peaks in Utah have 
surpassed approach the 'Unhealthy' category, in which some members of the
